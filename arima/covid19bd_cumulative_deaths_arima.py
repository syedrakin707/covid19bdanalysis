...
new_data = pd.DataFrame(index=range(0,len(df)),columns=['DATE', 'DEATHS_CF'])

##print(new_data)
for i in range(0,len(data)):
     new_data['DATE'][i] = data['DATE'][i]
     new_data['DEATHS_CF'][i] = data['DEATHS_CF'][i]
...
training = train['DEATHS_CF']
validation = valid['DEATHS_CF']
...
plt.figure(1)
plt.subplot(2,1,1)
plt.plot(train['DEATHS_CF'], color='b', label='Cumulative Deaths (Trained Data)')
plt.plot(valid['DEATHS_CF'], color='y', label='Cumulative Deaths (Validation Data Pre ARIMA)')
plt.plot(forecast_1['PRED'], color='r', label='Cumulative Deaths (Forecast Data Post ARIMA - Model 1)')
plt.legend()


valid_num = np.array(valid['DEATHS_CF'])
...
plt.plot(train['DEATHS_CF'], color='b', label='Cumulative Deaths (Trained Data)')
plt.plot(valid['DEATHS_CF'], color='y', label='Cumulative Deaths (Validation Data Pre ARIMA)')
plt.plot(forecast_2['PRED'], color='r', label='Cumulative Deaths (Forecast Data Post ARIMA - Model 2)')
plt.legend()
...
plt.plot(train['DEATHS_CF'], color='b', label='Cumulative Deaths (Trained Data)')
plt.plot(valid['DEATHS_CF'], color='y', label='Cumulative Deaths (Validation Data Pre ARIMA)')
plt.plot(forecast_3['PRED'], color='r', label='Cumulative Deaths (Forecast Data Post ARIMA - Model 3)')
plt.legend()
