...
new_data = pd.DataFrame(index=range(0,len(df)),columns=['DATE', 'CURED_DAILY'])

##print(new_data)
for i in range(0,len(data)):
     new_data['DATE'][i] = data['DATE'][i]
     new_data['CURED_DAILY'][i] = data['CURED_DAILY'][i]
...
training = train['CURED_DAILY']
validation = valid['CURED_DAILY']
...
plt.figure(1)
plt.subplot(2,1,1)
plt.plot(train['CURED_DAILY'], color='b', label='Daily Recoveries (Trained Data)')
plt.plot(valid['CURED_DAILY'], color='y', label='Daily Recoveries (Validation Data Pre ARIMA)')
plt.plot(forecast_1['PRED'], color='r', label='Daily Recoveries (Forecast Data Post ARIMA - Model 1)')
plt.legend()


valid_num = np.array(valid['CURED_DAILY'])
...
plt.plot(train['CURED_DAILY'], color='b', label='Daily Recoveries (Trained Data)')
plt.plot(valid['CURED_DAILY'], color='y', label='Daily Recoveries (Validation Data Pre ARIMA)')
plt.plot(forecast_2['PRED'], color='r', label='Daily Recoveries (Forecast Data Post ARIMA - Model 2)')
plt.legend()
...
plt.plot(train['CURED_DAILY'], color='b', label='Daily Recoveries (Trained Data)')
plt.plot(valid['CURED_DAILY'], color='y', label='Daily Recoveries (Validation Data Pre ARIMA)')
plt.plot(forecast_3['PRED'], color='r', label='Daily Recoveries (Forecast Data Post ARIMA - Model 3)')
plt.legend()
