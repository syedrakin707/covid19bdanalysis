...
new_data = pd.DataFrame(index=range(0,len(df)),columns=['DATE', 'CURED_CF'])

##print(new_data)
for i in range(0,len(data)):
     new_data['DATE'][i] = data['DATE'][i]
     new_data['CURED_CF'][i] = data['CURED_CF'][i]
...
training = train['CURED_CF']
validation = valid['CURED_CF']
...
plt.figure(1)
plt.subplot(2,1,1)
plt.plot(train['CURED_CF'], color='b', label='Cumulative Recoveries (Trained Data)')
plt.plot(valid['CURED_CF'], color='y', label='Cumulative Recoveries (Validation Data Pre ARIMA)')
plt.plot(forecast_1['PRED'], color='r', label='Cumulative Recoveries (Forecast Data Post ARIMA - Model 1)')
plt.legend()


valid_num = np.array(valid['CURED_CF'])
...
plt.plot(train['CURED_CF'], color='b', label='Cumulative Recoveries (Trained Data)')
plt.plot(valid['CURED_CF'], color='y', label='Cumulative Recoveries (Validation Data Pre ARIMA)')
plt.plot(forecast_2['PRED'], color='r', label='Cumulative Recoveries (Forecast Data Post ARIMA - Model 2)')
plt.legend()
...
plt.plot(train['CURED_CF'], color='b', label='Cumulative Recoveries (Trained Data)')
plt.plot(valid['CURED_CF'], color='y', label='Cumulative Recoveries (Validation Data Pre ARIMA)')
plt.plot(forecast_3['PRED'], color='r', label='Cumulative Recoveries (Forecast Data Post ARIMA - Model 3)')
plt.legend()
