...
new_data = pd.DataFrame(index=range(0,len(df)),columns=['DATE', 'CURED_DAILY_CIP'])

##print(new_data)
for i in range(0,len(data)):
     new_data['DATE'][i] = data['DATE'][i]
     new_data['CURED_DAILY_CIP'][i] = data['CURED_DAILY_CIP'][i]
...
training = train['CURED_DAILY_CIP']
validation = valid['CURED_DAILY_CIP']
...
plt.figure(1)
plt.subplot(2,1,1)
plt.plot(train['CURED_DAILY_CIP'], color='b', label='% Inc. in Daily Recoveries (Trained Data)')
plt.plot(valid['CURED_DAILY_CIP'], color='y', label='% Inc. in Daily Recoveries (Validation Data Pre ARIMA)')
plt.plot(forecast_1['PRED'], color='r', label='% Inc. in Daily Recoveries (Forecast Data Post ARIMA - Model 1)')
plt.legend()


valid_num = np.array(valid['CURED_DAILY_CIP'])
...
plt.plot(train['CURED_DAILY_CIP'], color='b', label='% Inc. in Daily Recoveries (Trained Data)')
plt.plot(valid['CURED_DAILY_CIP'], color='y', label='% Inc. in Daily Recoveries (Validation Data Pre ARIMA)')
plt.plot(forecast_2['PRED'], color='r', label='% Inc. in Daily Recoveries (Forecast Data Post ARIMA - Model 2)')
plt.legend()
...
plt.plot(train['CURED_DAILY_CIP'], color='b', label='% Inc. in Daily Recoveries (Trained Data)')
plt.plot(valid['CURED_DAILY_CIP'], color='y', label='% Inc. in Daily Recoveries (Validation Data Pre ARIMA)')
plt.plot(forecast_3['PRED'], color='r', label='% Inc. in Daily Recoveries (Forecast Data Post ARIMA - Model 3)')
plt.legend()
