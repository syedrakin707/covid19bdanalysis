...
new_data = pd.DataFrame(index=range(0,len(df)),columns=['DATE', 'CASES_DAILY_CIP'])

##print(new_data)
for i in range(0,len(data)):
     new_data['DATE'][i] = data['DATE'][i]
     new_data['CASES_DAILY_CIP'][i] = data['CASES_DAILY_CIP'][i]
...
training = train['CASES_DAILY_CIP']
validation = valid['CASES_DAILY_CIP']
...
plt.figure(1)
plt.subplot(2,1,1)
plt.plot(train['CASES_DAILY_CIP'], color='b', label='% Inc. in Daily Cases (Trained Data)')
plt.plot(valid['CASES_DAILY_CIP'], color='y', label='% Inc. in Daily Cases (Validation Data Pre ARIMA)')
plt.plot(forecast_1['PRED'], color='r', label='% Inc. in Daily Cases (Forecast Data Post ARIMA - Model 1)')
plt.legend()


valid_num = np.array(valid['CASES_DAILY_CIP'])
...
plt.plot(train['CASES_DAILY_CIP'], color='b', label='% Inc. in Daily Cases (Trained Data)')
plt.plot(valid['CASES_DAILY_CIP'], color='y', label='% Inc. in Daily Cases (Validation Data Pre ARIMA)')
plt.plot(forecast_2['PRED'], color='r', label='% Inc. in Daily Cases (Forecast Data Post ARIMA - Model 2)')
plt.legend()
...
plt.plot(train['CASES_DAILY_CIP'], color='b', label='% Inc. in Daily Cases (Trained Data)')
plt.plot(valid['CASES_DAILY_CIP'], color='y', label='% Inc. in Daily Cases (Validation Data Pre ARIMA)')
plt.plot(forecast_3['PRED'], color='r', label='% Inc. in Daily Cases (Forecast Data Post ARIMA - Model 3)')
plt.legend()
