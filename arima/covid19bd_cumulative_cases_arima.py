...
plt.title('Cumulative Confirmed Cases in Bangladesh (Used Auto Arima)')

data = df.sort_index(ascending=True, axis=0)
new_data = pd.DataFrame(index=range(0,len(df)),columns=['DATE', 'CASES_CF'])

##print(new_data)
for i in range(0,len(data)):
     new_data['DATE'][i] = data['DATE'][i]
     new_data['CASES_CF'][i] = data['CASES_CF'][i]
...
training = train['CASES_CF']
validation = valid['CASES_CF']
...
plt.figure(1)
plt.subplot(2,1,1)
plt.plot(train['CASES_CF'], color='b', label='Cumulative Cases (Trained Data)')
plt.plot(valid['CASES_CF'], color='y', label='Cumulative Cases (Validation Data Pre ARIMA)')
plt.plot(forecast_1['PRED'], color='r', label='Cumulative Cases (Forecast Data Post ARIMA - Model 1)')
plt.legend()


valid_num = np.array(valid['CASES_CF'])
...
plt.plot(train['CASES_CF'], color='b', label='Cumulative Cases (Trained Data)')
plt.plot(valid['CASES_CF'], color='y', label='Cumulative Cases (Validation Data Pre ARIMA)')
plt.plot(forecast_2['PRED'], color='r', label='Cumulative Cases (Forecast Data Post ARIMA - Model 2)')
plt.legend()
...
plt.plot(train['CASES_CF'], color='b', label='Cumulative Cases (Trained Data)')
plt.plot(valid['CASES_CF'], color='y', label='Cumulative Cases (Validation Data Pre ARIMA)')
plt.plot(forecast_3['PRED'], color='r', label='Cumulative Cases (Forecast Data Post ARIMA - Model 3)')
plt.legend()
