...
new_data = pd.DataFrame(index=range(0,len(df)),columns=['DATE', 'DEATHS_CF_CIP'])

##print(new_data)
for i in range(0,len(data)):
     new_data['DATE'][i] = data['DATE'][i]
     new_data['DEATHS_CF_CIP'][i] = data['DEATHS_CF_CIP'][i]
...
training = train['DEATHS_CF_CIP']
validation = valid['DEATHS_CF_CIP']
...
plt.figure(1)
plt.subplot(2,1,1)
plt.plot(train['DEATHS_CF_CIP'], color='b', label='% Increase in Cumulative Deaths (Trained Data)')
plt.plot(valid['DEATHS_CF_CIP'], color='y', label='% Increase in Cumulative Deaths (Validation Data Pre ARIMA)')
plt.plot(forecast_1['PRED'], color='r', label='% Increase in Cumulative Deaths (Forecast Data Post ARIMA - Model 1)')
plt.legend()


valid_num = np.array(valid['DEATHS_CF_CIP'])
...
plt.plot(train['DEATHS_CF_CIP'], color='b', label='% Increase in Cumulative Deaths (Trained Data)')
plt.plot(valid['DEATHS_CF_CIP'], color='y', label='% Increase in Cumulative Deaths (Validation Data Pre ARIMA)')
plt.plot(forecast_2['PRED'], color='r', label='% Increase in Cumulative Deaths (Forecast Data Post ARIMA - Model 2)')
plt.legend()
...
plt.plot(train['DEATHS_CF_CIP'], color='b', label='% Increase in Cumulative Deaths (Trained Data)')
plt.plot(valid['DEATHS_CF_CIP'], color='y', label='% Increase in Cumulative Deaths (Validation Data Pre ARIMA)')
plt.plot(forecast_3['PRED'], color='r', label='% Increase in Cumulative Deaths (Forecast Data Post ARIMA - Model 3)')
plt.legend()
